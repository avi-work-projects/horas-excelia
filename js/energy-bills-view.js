...
function openEnergyBills(kind){
  ...
  w.querySelector('#energyBillsBack').onclick=function(){cerrarPanel('energyBillsWrap','energyBillsOverlay');};
...
function openEnergyBill(kind,id){
  ...
  h+='</div><p class="energy-caption">Deja consumo o cargo vacíos si no constan. Los abonos admiten importes negativos.</p><label class="energy-field">Nota<textarea id="eb-notes">'+escHtml(b.notes)+'</textarea></label><div class="ev-io-row">'+(id?'<button class="ev-io-btn io-peligro" id="energyBillDelete">Eliminar</button>':'')+'<button class="ev-io-btn io-primaria" id="energyBillSave">Guardar</button></div>';
