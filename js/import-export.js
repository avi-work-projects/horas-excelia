...
var _g4=document.getElementById('exportAllBtn'); if(_g4)_g4.addEventListener('click',function(){
  ...
  var data={version:7,calendarNotes:appStorage.getItem(EV_ICS_NOTES_KEY)==='true',calendarAuthor:evIcsAuthor(),calendarExports:evIcsRecords(),csvExports:csvExportRecords(),bodaConfig:JSON.parse(JSON.stringify(BODA_CONFIG)),mailConfig:_lsJson(MAIL_CFG_SK,null),days:ST,sent:SW,monthH:MONTH_H,rate:DAILY_RATE,
...
function _applyFullImport(d,mode){
    ...
    try{
      ...
      if(d.calendarExports||!merge)appStorage.setItem(EV_ICS_KEY,JSON.stringify(merge?evIcsMergeRecords(evIcsRecords(),d.calendarExports||{}):(d.calendarExports||{})));
