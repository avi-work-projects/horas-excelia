...
function energyConsumptionHtml(kind,months,year){
  ...
  months.forEach(function(m,i){var days=Object.keys(m.days).length,valid=days&&!m.unknownConsumption&&!m.overlap;h+='<tr><td>'+MN_SHORT[i]+'</td><td>'+energyNumber(valid?m.consumption:null,'')+'</td>';if(kind==='luz')m.periods.forEach(function(v){h+='<td>'+energyNumber(valid&&m.periodDays===days?v:null,'')+'</td>';});h+='</tr>';});
  ...
  return h+'</tbody></table></details><p class="energy-caption">Los acumulados se distribuyen por días entre lecturas; no se extrapolan días sin cubrir.'+(kind==='luz'?' Si falta desglose por tramos no se reparte a partes iguales.':' Una cuota fija sin nueva lectura permite calcular el coste, pero no implica un consumo de cero.')+'</p><details class="energy-contract"><summary>Facturas de origen</summary>'+energyArchiveHtml(kind)+'</details>';
}
function energyCostsHtml(kind,year){
  ...
  h+='<section class="energy-contract"><h3>Estimado frente a facturas · '+year+'</h3><div class="energy-metrics">'+energyMetric('Estimado · IVA real',energyNumber(comparison.estimate,'€'),comparison.days+' días comparables')+energyMetric('Facturado · mismo período',energyNumber(total,'€'),'Solo suministro; importes prorrateados por días')+energyMetric('Diferencia estimado − facturado',energyNumber(diff,'€'),diff!==null&&total!==0?(diff/Math.abs(total)*100).toFixed(1)+' %':'')+'</div><p class="energy-caption">Mismos días de consumo, aunque la factura o el abono se emitiesen en otro año. Se excluyen mantenimiento y otros servicios ajenos al suministro. La comparación siempre usa IVA histórico; las cuotas fijas pueden calcularse sin lectura de consumo.'+(comparison.omitted?' Hay '+comparison.omitted+' días excluidos por datos incompletos.':'')+'</p><details><summary>Totales por fecha de emisión</summary><p>Suministro: '+energyNumber(comparison.issuedSupply,'€')+'</p><p>Servicios adicionales: '+energyNumber(comparison.services,'€')+'</p></details></section>';
